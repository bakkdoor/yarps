...
class ProjectMembershipsControllerTest < ActionController::TestCase
  ...
  def test_should_show_project_membership
    get :show, :id => project_memberships(:non_private_and_none_invite_only).id
    assert_response :success
  end

  def test_should_get_edit
    get :edit, :id => project_memberships(:non_private_and_none_invite_only).id
    assert_response :success
  end

  def test_should_update_project_membership
    put :update, :id => project_memberships(:non_private_and_none_invite_only).id, :project_membership => { }
    assert_redirected_to project_membership_path(assigns(:project_membership))
  end

  def test_should_destroy_project_membership
    assert_difference('ProjectMembership.count', -1) do
      delete :destroy, :id => project_memberships(:non_private_and_none_invite_only).id
    end
